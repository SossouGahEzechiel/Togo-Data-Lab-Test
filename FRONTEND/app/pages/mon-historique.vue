<template>
	<div class="space-y-6">
		...
		<div class="bg-white dark:bg-dark-800 rounded-lg shadow overflow-hidden">
			...
			<div v-else>
				<!-- Tableau desktop -->
				<ReservationListView :isManageMode="false" :filteredReservations="filteredReservations"
				...
				<!-- Liste mobile -->
				<ReservationCardView :isManageMode="false" :filteredReservations="filteredReservations"
...
import ReservationFilters from './ReservationFilters.vue'
...
const loadReservations = async () => {
	try {
		await reservationStore.findAll(useAuthStore().user?.id);
	} catch (error) {
...
// Lifecycle
onMounted(() => {
	setPageTitle('Historique', 'Mon historique de réservations')
	loadReservations()
})
</script>
